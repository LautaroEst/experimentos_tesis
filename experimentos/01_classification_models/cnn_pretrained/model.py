...
def init_model(vocab,embeddings,filter_sizes,nclasses,n_filters,dropout):
    
    embedding_dim = 300
    model = CNNModel(len(vocab),embedding_dim,filter_sizes,n_filters,nclasses,dropout)
...
class Classifier(object):
    ...
    def __init__(self,nclasses,pattern,frequency_cutoff,max_tokens,max_sent_len,
                embeddings,filter_sizes,n_filters,dropout,batch_size,
    ...
    def train(self,ds,y,eval_every=1,dev=None):
        ...
        model = init_model(self.vec.vocab,self.embeddings,self.filter_sizes,
                            self.nclasses,self.n_filters,self.dropout)
        model.to(device)
        model.train()
